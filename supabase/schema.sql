...
-- =========================================================
-- 21. PRACTICE QUIZZES & KNOWLEDGE CHECKS
-- =========================================================
...
create table if not exists public.practice_quizzes (
  ...
  title text not null,
  description text,
  estimated_minutes integer not null default 5 check (estimated_minutes >= 1),
  is_published boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create index if not exists practice_quizzes_lesson_idx on public.practice_quizzes(lesson_id);

drop trigger if exists practice_quizzes_set_updated_at on public.practice_quizzes;
...
create index if not exists practice_questions_quiz_idx on public.practice_questions(quiz_id, position);
...
create index if not exists practice_question_options_question_idx on public.practice_question_options(question_id, position);

create table if not exists public.practice_question_correct_options (
  question_id uuid not null references public.practice_questions(id) on delete cascade,
  option_id uuid not null references public.practice_question_options(id) on delete cascade,
  primary key(question_id, option_id)
);

create index if not exists practice_question_correct_options_q_idx on public.practice_question_correct_options(question_id);
...
create index if not exists practice_quiz_attempts_user_idx on public.practice_quiz_attempts(user_id, quiz_id);

create table if not exists public.practice_quiz_answers (
  id uuid primary key default gen_random_uuid(),
  attempt_id uuid not null references public.practice_quiz_attempts(id) on delete cascade,
  question_id uuid not null references public.practice_questions(id) on delete cascade,
...
create index if not exists practice_quiz_answers_attempt_idx on public.practice_quiz_answers(attempt_id);
...
-- Quiz RLS Policies
create policy "Public can read published practice quizzes" on public.practice_quizzes for select to anon, authenticated using (is_published = true);
create policy "Public can read published practice questions" on public.practice_questions for select to anon, authenticated using (true);
create policy "Public can read published practice question options" on public.practice_question_options for select to anon, authenticated using (true);
create policy "Users can read own quiz attempts" on public.practice_quiz_attempts for select to authenticated using (auth.uid() = user_id);
create policy "Users can create own quiz attempts" on public.practice_quiz_attempts for insert to authenticated with check (auth.uid() = user_id);
create policy "Users can update own quiz attempts" on public.practice_quiz_attempts for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "Users can read own quiz answers" on public.practice_quiz_answers for select to authenticated using (exists (select 1 from public.practice_quiz_attempts a where a.id = practice_quiz_answers.attempt_id and a.user_id = auth.uid()));
create policy "Users can create own quiz answers" on public.practice_quiz_answers for insert to authenticated with check (exists (select 1 from public.practice_quiz_attempts a where a.id = practice_quiz_answers.attempt_id and a.user_id = auth.uid()));
